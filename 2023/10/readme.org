...
the second uses a trivial kind of fubini's theorem to calculate the area: i
calculate the area along each horizontal slice and sum the areas. a crossing and
parity argument is used to calculate the sectional area. we start from the left,
outside of the bounded region, and move to the right. we transition from the
inside to the outside, or from the outside to the inside, if and only if we
cross the boundary. the boundary. when we are on the inside, we count tiles;
otherwise, we don't. it's a bit like a (similarly trivial) form of the jordan
curve theorem.
