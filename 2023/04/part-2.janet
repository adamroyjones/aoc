...
(defn matching-numbers [parsed-line]
  (assert (= (length parsed-line) 3))
  (def winning-numbers (set/new (parsed-line 1)))
  (def ticket-numbers (parsed-line 2))
  (count |(not (nil? (get winning-numbers $))) ticket-numbers))

(test (matching-numbers (parse-line "Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53")) 4)
(test (matching-numbers (parse-line "Card 2: 13 32 20 16 61 | 61 30 68 82 17 32 24 19")) 2)
(test (matching-numbers (parse-line "Card 3:  1 21 53 59 44 | 69 82 63 72 16 21 14  1")) 2)
(test (matching-numbers (parse-line "Card 4: 41 92 73 84 69 | 59 84 76 51 58  5 54 83")) 1)
(test (matching-numbers (parse-line "Card 5: 87 83 26 28 32 | 88 30 70 12 93 22 82 36")) 0)
(test (matching-numbers (parse-line "Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11")) 0)
...
(defn state-reducer [cur-state parsed-line]
  ...
  (def position (first parsed-line))
  (def card-count (cur-state position))
  (def max-step (matching-numbers parsed-line))
...
(defn solve [filename]
  (def parsed-lines (->> filename (slurp) (string/trim) (string/split "\n") (map parse-line)))
